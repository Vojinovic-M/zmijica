...
class SupabaseMonad {
    constructor(client) { this.client = client; this.promise = Promise.resolve(); }
    static of(client) { return new SupabaseMonad(client); }
    chain(fn) {
    ...
    fork(success, failure) { this.promise.then(success).catch(failure); }
}
